...
/*
setshowcal(true) runs immediately when React executes your component function.

This changes state while React is still figuring out what to render.

State change triggers another render immediately.

Which again calls setshowcal(true).

Infinite loop 🔁 → "Too many re-renders" error.

3. Why should it be a callback function?
...
() => setshowcal(true) is just a function reference.

Nothing runs yet. React just stores it and says:
"When the user actually clicks, I’ll call this function."

Rendering finishes without changing state.

Later, when the user clicks → React executes the callback → updates state → re-renders → ✅ works correctly.
*/
  useEffect(() => {
